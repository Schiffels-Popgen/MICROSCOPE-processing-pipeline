...
df_list <- get_df_list(
  c("TAB_Protocol", "TAB_Sequencing_Sequencer", "TAB_Site","TAB_Individual","TAB_Library", "TAB_Sequencing","TAB_Raw_Data"),
  con)
...
#   left_join(.,df_list[["TAB_Library"]], by=c("Full_Individual_Id"="Sample_Name")) %>% 
...
#     seq_kit=ifelse(num_fq == num_r1, "SE", "PE")) %>%
...
results <- inner_join(df_list[["TAB_Sequencing"]], query_list_seq, by=c("Full_Sequencing_Id"="Sequencing")) %>% 
  ...
  separate(`FastQ_Files`, into=c("R1", "R2"), sep=";", fill="right") %>%
   mutate(Lane=as.integer(str_replace(`R1`,"[[:graph:]]*_L([[:digit:]]{3})_R[[:graph:]]*", "\\1")), Strandedness=map_chr(`Protocol`, function (.) {infer_library_specs(.)[1]}), UDG_Treatment=map_chr(`Protocol`, function(.){infer_library_specs(.)[2]}), Colour_Chemistry=map_int(`Sequencer`, infer_color_chem))
