...
# sex_cutoffs <- matrix(c(xx_xmin, xx_xmax, xx_ymin, xx_ymax, xy_xmin, xy_xmax, xy_ymin, xy_ymax), nrow=2, byrow=T, dimnames = list(c("XX","XY"), c("xmin", "xmax","ymin","ymax")))

## Function to infer genetic sex from x/y rates
infer_sex <- function(sex_det, sex_cutoffs){
  print(sex_cutoffs[1,"xmin"])
  sex_det <- sex_det %>%
    mutate("Genetic Sex" = ifelse(
      `x-rate` > sex_cutoffs[1,"xmin"] & 
        `x-rate` < sex_cutoffs[1,"xmax"] & 
        `y-rate` > sex_cutoffs[1,"ymin"] & 
        `y-rate` < sex_cutoffs[1,"ymax"], "F",
      ifelse(`x-rate` > sex_cutoffs[2,"xmin"] & 
               `x-rate` < sex_cutoffs[2,"xmax"] & 
               `y-rate` > sex_cutoffs[2,"ymin"] & 
               `y-rate` < sex_cutoffs[2,"ymax"], "M",
             "Unknown"))) %>%
