...
if (file.exists(params$read_results_fn) && file.exists(params$read_plot_fn)) {
  read_results <- read_tsv(params$read_results_fn, col_types = 'ccdd') %>%
    mutate(
      ...
      ## If ssDNA data, the _ss suffix is present so ind2 id is shifted over a few characters.
      IndB_ID = case_when(
        keep_only == 'doublestrand' ~ substr(PairIndividuals, 7,12),
        keep_only == 'singlestrand' ~ substr(PairIndividuals, 10,15),
        TRUE ~ NA_character_
      ),
