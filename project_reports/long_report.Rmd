...
params: 
    ...
    snp_coverage_file: "~/mount/eager_outputs/2021-09-09-potzneusiedl/genotyping/single_eigenstrat_coverage.txt"
    sex_det_file: "~/mount/eager_outputs/2021-09-09-potzneusiedl/sex_determination/SexDet.txt"
    stats_table: "~/mount/eager_outputs/2021-09-09-potzneusiedl/multiqc/multiqc_data/multiqc_general_stats.txt"
    poseidon_geno: "~/mount/poseidon_packages/2021-09-09-potzneusiedl/2021-09-09-potzneusiedl.geno.txt"
    poseidon_snp: "~/mount/poseidon_packages/2021-09-09-potzneusiedl/2021-09-09-potzneusiedl.snp.txt"
    poseidon_ind: "~/mount/poseidon_packages/2021-09-09-potzneusiedl/2021-09-09-potzneusiedl.ind.txt"
    poseidon_janno: "~/mount/forged_packages/microscope_pca/microscope_pca.janno"
    pmmr_results_fn: "~/mount/automated_analysis/2021-09-09-potzneusiedl/pmmr/2021-09-09-potzneusiedl.pmmr.txt"
...
```{r echo=FALSE, fig.height=5, fig.width=8.25, message=FALSE}
pmmr_data<-read_tsv(params$pmmr_results_fn) %>%
  ...
  filter(!is.na(pMismatch) )#%>% ## Remove rows with p Mismatch == NA which are invdividuals with 0 overlapping SNPs. 
  # filter(!grepl(paste(bad_cov_inds,collapse="|"), Label))
...
## Load list of WEur populations
# west_eurasian_pops <- read_tsv("~/mount/forged_packages/microscope_pca/west_eurasian_poplist.txt", col_names = "Pop", col_types = 'c') 
```
...
# dm <- na.omit(dm) ## Remove rows of NAs (when an ancient individual has no coverage)
```
...
```{r MDS and plot, echo=FALSE, fig.height=5, fig.width=7}
...
ggplot()+
  coord_equal() +
  geom_text(data=components %>% filter(bg==T), aes(x=PC1, y=PC2, label=label, col=factor(colour)), alpha=0.4, size=2, show.legend=F) +
  ...
  theme_minimal() +
  geom_point(data=components %>% filter(bg==F), aes(x=PC1, y=PC2, shape=Ind)) +
  xlab(paste0("PC1")) +
  ylab(paste0("PC2")) +
