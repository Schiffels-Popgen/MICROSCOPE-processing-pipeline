...
params: 
    ...
    poseidon_geno: "~/mount/poseidon_packages/2020-05-03-relic_samples/2020-05-03-relic_samples.geno"
    poseidon_snp: "~/mount/poseidon_packages/2020-05-03-relic_samples/2020-05-03-relic_samples.snp"
    poseidon_ind: "~/mount/poseidon_packages/2020-05-03-relic_samples/2020-05-03-relic_samples.ind"
    poseidon_janno: "~/mount/forged_packages/microscope_pca/microscope_pca.janno"
    read_results_fn: "~/mount/automated_analysis/2020-05-03-relic_samples/read/2020-05-03-relic_samples.read.txt"
...
header-includes: 
 ...
 \pagestyle{fancy}
 \fancyhead[R]{\includegraphics[height = 1.5cm]{/Users/lamnidis/MPI_Logo_ENG_CMYK_green.png}}
 \fancyfoot{} 
...
\begin{center}
ERC-Project MICROSCOPE. Sample Report.
...
The proportion of Endogenous DNA is a measure of human DNA preservation.
...
The presence of DNA damage helps confirm that the DNA in a sequenced library is in fact ancient. Libraries with low proportion of DNA damage (Damage 1st bp 5\') and high endogenous DNA are potentially contaminated with present-day DNA molecules.
...
```{r import pMMR, echo=FALSE, fig.height=5, fig.width=8.25, message=FALSE}
...
## Calculate median
median_pmmr <- median(pmmr_data$pMismatch)
## Filter out lowest coverage results. 
pmmr_data <- pmmr_data %>% filter(nSNPs > red_below_x_snps)

## Dynamic figure size based on number of points.
fig_width=min(max(1+(0.2*nrow(pmmr_data)), 6), 225)
fig_height=max(5/8.25*fig_width, 5)
show_pmmr <- nrow(pmmr_data) > 0
```
\newif\ifpmmr
\pmmr`r ifelse(show_pmmr, 'true', 'false')`
\ifpmmr
The pairwise genotype mismatch rate (pMMR) between different individuals, can be indicative of genetic kinship. Unrelated individuals will have a higher pMMR, with multiple unrelated pairs forming a plateau around the median pMMR value. The pMMR of genetically related individuals is lowered proportionally to the closeness of their relationship. Some rough expected values for pMMR based on an underlying relationship are:

* **Identical Twins/ Same individual**: 50% of median value
* **First Degree**: 75% of median value
* **Second Degree**: 87.5% of median value
...
The colour of the points indicates the relative confidence around the pMMR. Gray points are less reliable, and black points are reliable estimates of the pMMR. The numbers above each point correspond to the number of SNPs used in the calculation of the pMMR between each pair of individuals. The higher this number is the more reliable the estimate.

\else
Not enough individuals with high enough SNP coverage to calculate pMMR (requires a minimum of 2 individuals each with a minimum of `r format(snp_cov_cutoff, digits = 0, big.mark = " ", decimal.mark = ",")` SNPs). pMMR results will be added here if/when additional sequencing data becomes available.
\fi

```{r plot pMMR, echo=FALSE, fig.height=fig_height, fig.width=fig_width, message=FALSE, eval=nrow(pmmr_data) > 0}
ggplot(pmmr_data, aes(x=fct_reorder(Label, pMismatch), y=pMismatch, col=factor(colour, levels=c(paste0("<",black_above_x_snps," SNPs"),paste0(black_above_x_snps,"+ SNPs"))))) +
  scale_color_manual(values=stats::setNames(c("grey","black"), c(paste0("<",black_above_x_snps," SNPs"), paste0(black_above_x_snps,"+ SNPs")))) +
  labs(color = "number of\noverlapping\nSNPs") +
  geom_hline(yintercept = median_pmmr, colour="gray70", lty=3) +
    geom_text(label="median", x=nrow(pmmr_data), y=median_pmmr, show.legend=F, size=2.5, nudge_y=0.1) +
  geom_point(size=2) + 
  geom_text(aes(label=nSNPs, y=pMismatch), size=1.5, angle=90, nudge_y=0.05) +
...
pops_for_text <- pop_labels %>% 
  group_by(region) %>% 
  summarise(pops=paste(pop, collapse=", "), n=n())
  
bg_plot_data <- inner_join(pca_evec, pop_labels, by="pop")
pca_samples <- pca_evec %>% filter(ind %in% good_cov_inds)

pca_eval <- read_tsv(params$eval_fn, col_names = "eigenvalues", col_types='d')
...
* **Caucasus**: \textcolor[HTML]{F8766D}{red} (`r pops_for_text[pops_for_text$region == "Caucasus",2]`)
* **Near East**: \textcolor[HTML]{A3A500}{olive} (`r pops_for_text[pops_for_text$region == "NEast",2]`)
* **South Europe**: \textcolor[HTML]{00B0F6}{light blue} (`r pops_for_text[pops_for_text$region == "SEur",2]`)
* **Central/North Europe**: \textcolor[HTML]{E76BF3}{magenta} (`r pops_for_text[pops_for_text$region == "NEur",2]`)
* **Northeastern/Eastern Europe**: \textcolor[HTML]{00BF7D}{green} (`r pops_for_text[pops_for_text$region == "NEEur",2]`)
...
\ifpca
\newpage
Below is a zoomed in plot of the same PCA results, focusing on Europe.
