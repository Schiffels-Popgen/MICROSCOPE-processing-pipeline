...
sex_det <- read_tsv(params$sex_det_file, col_names=c("#Sample", "#SnpsAut", "#SNPsX", "#SnpsY", "NrAut", "NrX", "NrY", "x-rate", "y-rate"), col_types='ciiiiiidd', skip=1) %>%
  filter(!grepl("doublestrand", `#Sample`)) %>% ## When both ssDNA and dsDNA libraries exist, only keep single stranded libs.
...
```{r plot pMMR, echo=FALSE, fig.height=fig_height, fig.width=fig_width, message=FALSE, eval=nrow(pmmr_data) > 1}
